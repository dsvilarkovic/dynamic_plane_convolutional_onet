...
class MeshEvaluator(object):
    ...
    def eval_mesh(self, mesh, pointcloud_tgt, normals_tgt,
        ...
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            if True: #! Remove walls and floors
                ...
                #print(pointcloud_tgt[:, 0].min(), pointcloud_tgt[:, 0].max())
                #print(pointcloud[:, 0].min(), pointcloud[:, 0].max())
                #print(pointcloud_tgt[])
                #print(pointcloud[:, 1].min())
                #print(z_min, z_max)
                #print(pointcloud[:, 2].min(), pointcloud[:, 2].max())
                #pdb.set_trace()
                ...
                #from src.utils.io import export_pointcloud
                #export_pointcloud(pointcloud_new, 'tmp_new.ply')
                #export_pointcloud(pointcloud_tgt, 'tmp_gt.ply')
                #pdb.set_trace()
            else:
    ...
    def eval_pointcloud(self, pointcloud, pointcloud_tgt,
        ...
        out_dict = {
            ...
            'recall-1': recall[9],
            'f-score-2': F[19],
            'precision-2': precision[19],
            'recall-2': recall[19],
            'f-score-12': F[11],
            'precision-12': precision[11],
            'recall-12': recall[11],
            'f-score-15': F[14],
            'precision-15': precision[14],
            'recall-15': recall[14]
        }
