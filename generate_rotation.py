...
parser.add_argument('--rotate-planes', action='store_true', help='To see if we need plane rotations')

args = parser.parse_args()
cfg = config.load_config(args.config, 'configs/default.yaml')
is_cuda = (torch.cuda.is_available() and not args.no_cuda)
rotate_planes = args.rotate_planes
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    if generate_mesh:
        t0 = time.time()
        #Part of the code where we rotate objects
        out,_, rotation = generator.generate_rotated_mesh(data, DEGREES = DEGREES, save_rotation_tensor = rotate_planes)
        ...
        with open(rotation_out_file, 'wb') as f:
